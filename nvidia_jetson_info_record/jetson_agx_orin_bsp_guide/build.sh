...
{
    echo ""
    echo "USAGE: $0 [-R] [-S] [-K] [-M] [-D] [-F] [-r FLAVOR] [-u USER] [-p PASSWORD] [-j JOBS] [-a]"
    echo ""
    ...
    echo "       -a = build all: RootFS + Sync + Kernel + Modules + DTBs (不含 flash)"
    echo ""
    echo "EXAMPLE:"
...
while getopts "RSKMDFr:u:p:j:a" arg; do
    case $arg in
...
build_kernel() {
    ...
    log_info "Step 4: 安裝 Kernel 及 In-tree Modules..."
...
# -----------------------------------------------------------------------------
# Step 5: Build & Install Out-of-Tree Modules
# -----------------------------------------------------------------------------
build_modules() {
    log_info "========== Step 5: 編譯並安裝 NVIDIA Out-of-Tree Modules =========="
...
flash_device() {
    ...
    if is_in_docker; then
        ...
        log_info "改用 flash.sh（純 USB 傳輸，不需要 NFS）..."
        sudo ./flash.sh jetson-agx-orin-devkit nvme0n1p1 || die "燒錄失敗"
    else
        log_info "Host 環境，使用 nvsdkmanager_flash.sh 燒錄 (NVMe)..."
        sudo ./nvsdkmanager_flash.sh --storage nvme0n1p1 || die "燒錄失敗"
    fi
